...
K5=$(grep '^VWORLD_KEY5=' .env | cut -d= -f2)
...
# 공시가격 캐싱(villa_price) — 좌표 끝난 건물의 공시가격을 채운다.
PP=$(sqlite3 data/naverreal.sqlite "SELECT COUNT(*) FROM villa_master WHERE status='ok' AND price_status='pending'")
if [ "${PP:-0}" -gt 0 ]; then
  for K in "$K1" "$K2" "$K3" "$K4" "$K5"; do
    [ -n "$K" ] && .venv/bin/python -u scripts/build_villa_price.py --concurrency 8 --key "$K" >> data/villa_price.log 2>&1
  done
fi
