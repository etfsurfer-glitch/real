"""기존 rentals 테이블의 매칭을 새 단지 인덱스로 재계산.

API 호출 없음. raw 컬럼에 저장된 응답을 다시 매칭해서 matched_* 갱신.
단지 마스터에 OPST 같은 새 카테고리를 추가했을 때 매칭률 끌어올리는 용도.

Run:
    python scripts/rematch_rentals.py
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--only-unmatched", action="store_true",
                   help="현재 unmatched 행만 다시 시도 (이미 매칭된 건 손대지 않음)")
...
def main() -> int:
    ...
    # Load rows grouped by sgg_cd
    where = " WHERE matched_complex_no IS NULL" if args.only_unmatched else ""
    cur = conn.execute(
        f"SELECT rental_id, sgg_cd, raw, manual_override FROM rentals{where}"
    )
    by_sgg: dict[str, list[tuple]] = defaultdict(list)
    skipped_manual = 0
    for rid, sgg_cd, raw, manual in cur.fetchall():
        if manual:
            skipped_manual += 1
            continue
        by_sgg[sgg_cd].append((rid, raw))
            ...
            with rp_storage._LOCK:
                conn.executemany(
                    ...
                    SET matched_complex_no = ?,
                        ...
                        match_details      = ?,
                        matched_at         = ?
                    WHERE rental_id = ? AND manual_override = 0
                    """,
                    updates,
                )
                conn.commit()
    ...
    n_total = conn.execute("SELECT COUNT(*) FROM rentals").fetchone()[0]
    n_matched = conn.execute("SELECT COUNT(*) FROM rentals WHERE matched_complex_no IS NOT NULL").fetchone()[0]
    n_unmatched = n_total - n_matched
    print(f"\n[stats] rentals table after rematch:")
