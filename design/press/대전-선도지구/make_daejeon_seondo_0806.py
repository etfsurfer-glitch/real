...
SUBTITLE = ("콕집 전수 분석 — 매물 227→121건(-46.7%) · 둔산은 15개 타입 중 13개가 2021년 고점 넘어, "
            "송촌은 6개 타입 모두 아직 아래")
...
LEAD = (
    ...
    "8억 9,100만 원으로 7.3% 상승했다. 2021년 실거래 고점과 단지별·평형별로 하나씩 맞대어 보면 "
    "둔산 13·14구역은 비교 가능한 15개 타입 중 13개가 이미 고점을 넘었고 매물이 있는 14개 타입은 "
    "호가가 예외 없이 고점 위(중앙값 123%)인 반면, 송촌 6구역은 6개 타입 전부가 실거래도 호가도 "
    "아직 고점 아래(중앙값 88%·93%)다."
)
...
BODY = [
    ...
    ("2021년 고점과 견주면 — 둔산은 15개 타입 중 13개가 넘었고, 송촌은 6개 모두 아래",
     "지방 아파트값은 2021년에 정점을 찍고 내려온 곳이 많다. 대상 6개 단지도 2021년이 거래가 가장 "
     "활발했던 시기다. 그 해 실거래 고점과 올해 실거래를 단지별·평형별로 하나씩 맞대어 보면 두 구역이 "
     "정반대에 서 있다. 둔산 13·14구역은 비교 가능한 15개 타입 가운데 13개가 2021년 고점을 넘어섰다"
     "(회복률 중앙값 104%). 크로바 135㎡가 16억 9,500만 원에서 21억 4,000만 원으로 126%, 한가람 40㎡가 "
     "2억 8,000만 원에서 3억 5,000만 원으로 125%로 가장 높다. 넘지 못한 두 타입은 크로바 85㎡(92%)와 "
     "한가람 85㎡(95%)로, 공교롭게 둘 다 85㎡다. 반면 송촌 6구역은 6개 타입이 하나도 빠짐없이 고점 "
     "아래다. 보람 99㎡가 4억 3,000만 원에서 3억 3,500만 원으로 78%, 삼익소월 60㎡가 2억 1,000만 "
     "원에서 1억 7,300만 원으로 82%로 낮고, 가장 높은 삼익소월 85㎡도 95%에 그쳐 중앙값 88%다. "
     "‘둔산은 회복했다’고 뭉뚱그리기보다 타입별로 갈라 봐야 하는 이유다."),
    ("호가는 그보다 더 위 — 둔산은 매물 있는 14개 타입 전부가 고점 위",
     "지금 나와 있는 매물의 호가는 실거래보다 한 발 더 나가 있다. 둔산에서 매물이 있는 14개 타입은 "
     "예외 없이 2021년 고점 위에 호가가 붙어 있다. 범위는 107~143%, 중앙값 123%다. 한가람 40㎡가 "
     "2억 8,000만 원에서 4억 원으로 143%, 크로바 135㎡가 16억 9,500만 원에서 23억 5,000만 원으로 "
     "139%다. 실거래로는 고점을 못 넘은 크로바 85㎡와 한가람 85㎡도 호가는 각각 122%·111%로 이미 위에 "
     "있다. 실거래 회복률 중앙값(104%)과 견주면 호가가 20%포인트가량 앞서 있는 셈이다. 송촌 6구역은 "
     "호가마저 6개 타입 모두 고점 아래(81~95%, 중앙값 93%)다. 보람 99㎡ 호가가 3억 5,000만 원으로 "
     "고점(4억 3,000만 원)의 81%다. 다만 실거래 중앙값(88%)보다는 5%포인트 위여서, 송촌에서도 호가가 "
     "실거래를 앞서 부르는 방향은 같다. 같은 날 발표된 선도지구인데 시장이 두 구역을 전혀 다르게 값 "
     "매기고 있다는 뜻이다."),
    ...
    ("해석 시 유의점",
     ...
     "제거하지는 못한다. 실거래로 확인되기 전까지 호가는 매도 희망가일 뿐이다. 2021년 고점 비교도 "
     "타입별 최고가끼리 맞댄 것이라, 그 해 거래가 적었던 타입은 표본이 얇다. 같은 구역 안에서도 "
     "타입에 따라 회복률이 92~126%로 갈리므로 구역 단위로 뭉뚱그린 해석은 피하는 편이 좋다."),
...
METHOD = (
    ...
    "(해제거래 제외) 기준. 2021년 고점은 국토교통부 아파트 매매 실거래 2021년 1~12월 전수(6개 단지 "
    "287건)에서 단지별·평형(전용㎡ 반올림)별 최고가를 취한 값이며, 그 해 거래가 2건 미만인 평형은 "
    "고점으로 보기 어려워 제외. 회복률=올해 같은 타입 최고 실거래÷2021년 고점, 호가 비율=현재 같은 타입 "
    "최고 호가÷2021년 고점. 매물의 전용면적 표기는 실거래와 소수점이 달라 ±1.5㎡ 이내를 같은 타입으로 "
    "봄. 지금 매물이 없는 타입(목련 76㎡)은 호가 비교에서 제외. "
    "호가는 매도 희망가로 실거래 가격이 아님. 그 외 수치는 콕집 자체 구축 DB 실측(2026-08-06)."
)
...
# 구역, 단지, 전용㎡, 2021 고점(억), 2021 거래건수, 올해 최고 실거래(억), 회복률, 현재 최고 호가(억), 호가/고점
# 국토부 2021년 전수 287건 + 콕집 DB 실측(2026-08-06). 2021년 거래 2건 미만 평형은 제외.
RECOVERY = [
    ("둔산 13", "목련", "76", "7.00", "3", "8.00", "114%", "—", "—"),
    ("둔산 13", "목련", "102", "11.50", "10", "11.97", "104%", "15.00", "130%"),
    ("둔산 13", "목련", "118", "13.50", "7", "13.98", "104%", "18.00", "133%"),
    ("둔산 13", "목련", "135", "14.90", "12", "15.50", "104%", "18.00", "121%"),
    ("둔산 13", "크로바", "85", "12.50", "5", "11.50", "92%", "15.20", "122%"),
    ("둔산 13", "크로바", "102", "13.40", "11", "13.98", "104%", "16.50", "123%"),
    ("둔산 13", "크로바", "115", "15.90", "8", "16.20", "102%", "17.00", "107%"),
    ("둔산 13", "크로바", "135", "16.95", "4", "21.40", "126%", "23.50", "139%"),
    ("둔산 13", "크로바", "165", "20.80", "6", "23.50", "113%", "25.50", "123%"),
    ("둔산 14", "한가람", "40", "2.80", "44", "3.50", "125%", "4.00", "143%"),
    ("둔산 14", "한가람", "52", "3.95", "12", "4.38", "111%", "4.60", "116%"),
    ("둔산 14", "한가람", "60", "4.39", "8", "4.95", "113%", "5.60", "128%"),
    ("둔산 14", "한가람", "85", "6.50", "13", "6.20", "95%", "7.20", "111%"),
    ("둔산 14", "공작한양", "73", "5.00", "12", "5.05", "101%", "5.70", "114%"),
    ("둔산 14", "공작한양", "85", "6.50", "33", "6.70", "103%", "7.00", "108%"),
    ("송촌 6", "보람", "59", "2.49", "11", "2.20", "88%", "2.35", "94%"),
    ("송촌 6", "보람", "83", "3.19", "20", "2.97", "93%", "3.00", "94%"),
    ("송촌 6", "보람", "99", "4.30", "23", "3.35", "78%", "3.50", "81%"),
    ("송촌 6", "보람", "125", "4.70", "8", "4.15", "88%", "4.20", "89%"),
    ("송촌 6", "삼익소월", "60", "2.10", "30", "1.73", "82%", "2.00", "95%"),
    ("송촌 6", "삼익소월", "85", "2.83", "7", "2.70", "95%", "2.60", "92%"),
]
REC_HEAD = ["구역", "단지", "전용㎡", "2021 고점", "2021 건수", "올해 실거래", "회복률", "현재 최고호가", "호가/고점"]
REC_TITLE = "■ 단지별·타입별 2021년 고점 대비 (실거래·호가, 억 원)"
REC_AFTER = "2021년 고점과 견주면"          # 이 절 뒤에 표를 놓는다
...
def make_docx(out: Path):
    ...
    for head, body in BODY:
        ...
        if not head.startswith(REC_AFTER):
            continue
        h = d.add_paragraph(); r = h.add_run(REC_TITLE)
        r.font.bold = True; r.font.size = Pt(11.5)
        rt = d.add_table(rows=1, cols=len(REC_HEAD))
        rt.style = "Light Grid Accent 1"
        for c, txt in zip(rt.rows[0].cells, REC_HEAD):
            c.paragraphs[0].add_run(txt).bold = True
        prev = None
        for row in RECOVERY:
            cells = rt.add_row().cells
            vals = ((row[0] if row[0] != prev else ""),) + row[1:]
            prev = row[0]
            for i, (c, txt) in enumerate(zip(cells, vals)):
                run = c.paragraphs[0].add_run(txt)
                if i in (6, 8) and txt != "—":
                    run.bold = True
                    run.font.color.rgb = (RGBColor(0x12, 0x68, 0xD3)
                                          if int(txt.rstrip("%")) >= 100 else RGBColor(0xCC, 0x3D, 0x3D))
        for rowx in rt.rows:
            for c in rowx.cells:
                for pp in c.paragraphs:
...
def make_pdf(out: Path):
    ...
    def recovery_table():
        ss["cellc"] = ParagraphStyle("cellc", parent=ss["cell"], alignment=1)
        ss["cellcb"] = ParagraphStyle("cellcb", parent=ss["cellb"], alignment=1)
        data = [[Paragraph(x, ss["cellcb"]) for x in REC_HEAD]]
        styl = [
            ("BACKGROUND", (0, 0), (-1, 0), colors.Color(0.91, 0.945, 0.988)),
            ("GRID", (0, 0), (-1, -1), 0.5, colors.Color(0.85, 0.88, 0.92)),
            ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
        ...
        up, down = "#1268D3", "#CC3D3D"
        prev = None
        for r in RECOVERY:
            i = len(data)
            # 구역은 바뀔 때만 적어 눈이 덩어리로 읽게 한다
            vals = list((r[0] if r[0] != prev else "",) + r[1:])
            if r[0] != prev and prev is not None:
                styl.append(("LINEABOVE", (0, i), (-1, i), 1.0, colors.Color(*BLUE)))
            prev = r[0]
            for col in (6, 8):                       # 회복률·호가비 — 100% 기준으로 색을 나눈다
                v = vals[col]                        # 색은 문단 마크업으로 — TableStyle TEXTCOLOR는
                if v != "—":                         # Paragraph 셀엔 먹지 않는다
                    c = up if int(v.rstrip("%")) >= 100 else down
                    vals[col] = f'<font color="{c}"><b>{v}</b></font>'
            data.append([Paragraph(x, ss["cellc"]) for x in vals])
        t = Table(data, repeatRows=1,
                  colWidths=[17 * mm, 22 * mm, 15 * mm, 21 * mm, 18 * mm, 22 * mm, 17 * mm, 24 * mm, 18 * mm])
    ...
    for head, body in BODY:
        ...
        if head.startswith(REC_AFTER):
            story.append(Paragraph(REC_TITLE, ss["head"]))
