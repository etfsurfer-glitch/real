...
A(P("세제개편안 열흘, 서울 아파트 시장은 <b>어디로 가고 있나</b>", "title"))
A(P(f"서울 아파트 100채 가운데 열흘 새 값을 내린 집은 "
    f"<b>{seoul_p['발표후_인하비율']}채</b>. 30억 이상만 "
    f"{b30['발표후_인하비율']}채로, <b>내린 집이 올린 집보다 "
    f"{b30['발표후_인하대인상_배']}배 많다</b>. 그사이 매물은 전 구간에서 쌓였다"
    f"(30억 이상 {sign(st30['발표후_증감률'], unit='%', flip=True, bold=False)})", "sub"))
...
A(P(
    ...
    "아래는 두 기준으로 각각 본 결과이고, 마지막에 둘을 겹쳐 지금 시장이 어디로 가고 있는지를 "
    "적었다.", "lead"))
...
_h1 = [P("1. 기준 ① 사건 — 값을 고친 사람들은 어느 쪽으로 기울었나", "h"),
       P("지표는 <b>인상률 = 인상 ÷ (인상+인하)</b>다. 100%에 가까울수록 값을 고친 사람 중 "
         "올린 쪽이 많다는 뜻이다.", "body")]
rows = [hdr(["구분", "발표 전 10일", "발표 후 10일", "변화", "발표 후<br/>건수"])]
for k in AREAS:
    r = EV_O[k]
    rows.append([P(f"<b>{k}</b>", "tdl"), P(pc1(r["발표전_인상률"]), "td"),
                 P(f"<b>{pc1(r['발표후_인상률'])}</b>", "tdb"),
                 P(sign(r["발표전후_변화p"]), "td"), P(n(r["발표후_건수"]), "td")])
t = Table(rows, colWidths=[40 * mm, 30 * mm, 30 * mm, 26 * mm, 26 * mm], repeatRows=1)
t.setStyle(TableStyle(list(TBL) + [("BACKGROUND", (0, 2), (-1, 2), ROWALT),
                                   ("BACKGROUND", (0, 3), (-1, 3), HOTBG)]))
A(KeepTogether(_h1 + [t]))

rows = [hdr(["서울 가격대", "발표 전 10일", "발표 후 10일", "변화", "발표 후<br/>건수"])]
for b in BANDS:
    r = EV_B[b]
    rows.append([P(f"<b>{b}</b>", "tdl"), P(pc1(r["발표전_인상률"]), "td"),
                 P(f"<b>{pc1(r['발표후_인상률'])}</b>", "tdb"),
                 P(sign(r["발표전후_변화p"]), "td"), P(n(r["발표후_건수"]), "td")])
t = Table(rows, colWidths=[40 * mm, 30 * mm, 30 * mm, 26 * mm, 26 * mm], repeatRows=1)
st = list(TBL)
...
A(t)
A(P("붉게 칠한 줄은 발표 전후로 5%p 넘게 꺾인 구간이다.", "cap"))
...
# ── 2. 물건 기준 ────────────────────────────────────────────────────────
A(Spacer(1, 5))
_h2 = [P("2. 기준 ② 물건 — 시장 전체는 얼마나 움직였나", "h"),
       P("‘100채 중 내린 집’은 열흘 뒤 호가가 낮아진 물건의 수다(값을 안 고친 집도 분모에 "
         "들어간다). ‘내린 집 : 올린 집’이 <b>1.00배를 넘으면 내리는 쪽이 더 많다</b>는 뜻이다.",
         "body")]
rows = [hdr(["구분", "100채 중 내린 집<br/>발표 전 → 발표 후", "변화",
             "내린 집 : 올린 집<br/>발표 전 → 발표 후", "내릴 때<br/>깎는 폭", "물건 수"])]
for k in AREAS:
    r = PN_O[k]
    rows.append([P(f"<b>{k}</b>", "tdl"),
                 P(f"{r['발표전_인하비율']}채 → <b>{r['발표후_인하비율']}채</b>", "td"),
                 P(sign(r["발표전후_인하비율변화p"], flip=True), "td"),
                 P(f"{float(r['발표전_인하대인상_배']):.2f} → "
                   f"{ratio(r['발표후_인하대인상_배'])}", "td"),
                 P(cut(r["발표후_인하폭_중앙_만원"], r["발표후_인하폭_중앙_률"]), "td"),
                 P(n(r["발표후_물건수"]), "td")])
t = Table(rows, colWidths=[32 * mm, 36 * mm, 16 * mm, 38 * mm, 26 * mm, 20 * mm], repeatRows=1)
t.setStyle(TableStyle(list(TBL) + [("BACKGROUND", (0, 2), (-1, 2), ROWALT),
                                   ("BACKGROUND", (0, 3), (-1, 3), HOTBG)]))
A(KeepTogether(_h2 + [t]))

rows = [hdr(["서울 가격대", "100채 중 내린 집<br/>발표 전 → 발표 후", "변화",
             "내린 집 : 올린 집<br/>발표 전 → 발표 후", "내릴 때<br/>깎는 폭", "물건 수"])]
for b in BANDS:
    r = PN_B[b]
...
A(box(
    f"<b>서울 100채 중 값을 내린 집은 {seoul_p['발표후_인하비율']}채, 올린 집은 "
    f"{seoul_p['발표후_인상비율']}채 — 나머지 {FLAT}채는 그대로 뒀다</b><br/>"
    f"· 내릴 때 깎는 폭은 중앙값으로 서울 "
    f"{int(float(seoul_p['발표후_인하폭_중앙_만원']))/10000:.1f}억"
    f"({seoul_p['발표후_인하폭_중앙_률']}%), 30억 이상 "
    f"{int(float(b30['발표후_인하폭_중앙_만원']))/10000:.1f}억"
    f"({b30['발표후_인하폭_중앙_률']}%)다. <b>깎는 비율 자체는 어느 구간이나 2% 안팎으로 "
    f"비슷하다</b> — 달라지는 것은 얼마나 많은 집이 내리느냐다.<br/>"
    f"· <b>내린 집이 올린 집보다 많아지는 선은 15억이다.</b> 10억 미만 "
    f"{float(b10['발표후_인하대인상_배']):.2f}배, 10~15억 "
    f"{float(PN_B['10~15억']['발표후_인하대인상_배']):.2f}배로 오히려 올리는 쪽이 많고, "
...
rows = [hdr(["권역", "중앙 호가", "100채 중 내린 집<br/>발표 전 → 발표 후",
             "내린 집 : 올린 집<br/>발표 후", "내릴 때<br/>깎는 폭", "물건 수"])]
...
for r in ZONE:
    rows.append([P(f"<b>{r['권역']}</b> <font size=7 color='#8B95A5'>{ZN.get(r['권역'],'')}</font>",
                 ...
                 P(cut(r["발표후_인하폭_중앙_만원"], r["발표후_인하폭_중앙_률"]), "td"),
                 P(n(r["발표후_물건수"]), "td")])
t = Table(rows, colWidths=[40 * mm, 18 * mm, 36 * mm, 26 * mm, 26 * mm, 18 * mm], repeatRows=1)
...
rows = [hdr(["자치구", "중앙 호가", "100채 중 내린 집<br/>발표 전 → 발표 후", "변화",
             "내린 집 : 올린 집<br/>발표 후", "내릴 때<br/>깎는 폭", "물건 수"])]
for r in GU:
    rows.append([P(f"<b>{r['자치구']}</b>", "tdl"), P(f"{r['기준_중앙호가_억']}억", "td"),
                 ...
                 P(cut(r["발표후_인하폭_중앙_만원"], r["발표후_인하폭_중앙_률"]), "td"),
                 P(n(r["발표후_물건수"]), "td")])
t = Table(rows, colWidths=[24 * mm, 18 * mm, 34 * mm, 16 * mm, 26 * mm, 28 * mm, 18 * mm],
          repeatRows=1)
st = list(TBL)
...
t.setStyle(TableStyle(st))
A(t)
A(P("왼쪽에 붉은 선이 그어진 세 줄이 강남3구다. ‘변화’와 ‘배수’는 다른 것을 말한다 — 용산구는 값을 내린 집이 줄었지만(−0.5p) 여전히 내린 집이 올린 집의 1.92배다. 대체로 중앙 호가가 높은 구가 위쪽에 모인다.", "cap"))
...
A(Spacer(1, 4))
A(KeepTogether([
    P("5. 두 기준을 겹쳐 보면 — 시장은 어디로 가고 있나", "h"),
    box(
        f"<b>① 값은 아직 안 내렸다.</b> 서울 아파트 100채 가운데 열흘 새 값을 내린 집은 "
        f"{seoul_p['발표후_인하비율']}채, 올린 집은 {seoul_p['발표후_인상비율']}채다. "
        f"나머지 <b>{FLAT}채는 값을 그대로 뒀다.</b> 내린 집도 깎은 폭은 중앙값 "
        f"{seoul_p['발표후_인하폭_중앙_률']}%에 그친다. "
        "<b>지금 벌어진 일은 가격 하락이 아니라 관망이다.</b><br/><br/>"
        f"<b>② 경계는 15억이다.</b> 15억 아래에서는 값을 올린 집이 더 많고"
        f"(10억 미만 {float(b10['발표후_인하대인상_배']):.2f}배), 15억을 넘으면 뒤집혀 "
        f"30억 이상은 내린 집이 올린 집보다 <b>{float(b30['발표후_인하대인상_배']):.2f}배</b> "
        "많다. 사건 기준(1절)에서 15억 위 세 구간이 두 자리로 꺾인 것과 같은 자리다. "
        "<b>두 방법이 서로 다른 분모를 쓰고도 같은 선을 가리켰다.</b><br/><br/>"
        f"<b>③ 깎는 폭은 어디나 비슷하다.</b> 내릴 때 깎는 비율은 10억 미만 "
        f"{b10['발표후_인하폭_중앙_률']}%, 30억 이상 {b30['발표후_인하폭_중앙_률']}%로 차이가 "
        f"없다. 금액으로는 {int(float(b10['발표후_인하폭_중앙_만원'])):,}만원과 "
        f"{int(float(b30['발표후_인하폭_중앙_만원']))//10000}억이지만 집값 대비로는 같다. "
        "<b>달라지는 것은 깎는 크기가 아니라 깎는 집의 수</b>다.<br/><br/>"
        "<b>④ 그런데 매물이 쌓이고 있다 — 이것이 앞으로의 압력이다.</b> 열흘 만에 서울 물건은 "
        f"10억 미만 {sign(st10['발표후_증감률'], unit='%', flip=True, bold=False)}, "
        f"30억 이상 {sign(st30['발표후_증감률'], unit='%', flip=True, bold=False)} 늘었다. "
        "<b>값을 안 내린 채 재고만 늘고 있다</b>는 것은 부르는 값과 사려는 값의 거리가 벌어지고 "
        "있다는 뜻이다. 이 상태가 이어지면 다음에 움직이는 것은 호가다.<br/><br/>"
        "<b>⑤ 다만 열흘은 짧다.</b> 이 자료는 발표 전 10일과 발표 후 10일만 견준 것이다. "
        "계절성이나 추세를 걸러낼 만큼 긴 기간이 아니므로, <b>여기서 읽어야 할 것은 폭이 아니라 "
        "가격대 사이의 온도 차</b>다.")]))
