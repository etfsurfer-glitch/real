...
A(KeepTogether([
    ...
    box(
        ...
        f"<b>③ 저가는 가격이 받쳐주고 있다.</b> 10억 미만은 값을 내린 집이 오히려 "
        f"{sign(b10['발표전후_인하비율변화p'], flip=True, bold=False)} 줄었고, 올린 집이 "
        f"내린 집보다 많다({float(b10['발표후_인하대인상_배']):.2f}배). 노도강은 "
        f"{float([r for r in ZONE if r['권역']=='노도강'][0]['발표후_인하대인상_배']):.2f}배로 "
        "서울에서 값을 올리는 쪽이 가장 두텁다. <b>이 구간에서는 발표의 자취를 찾기 어렵다.</b>"
        "<br/><br/>"
