...
※ 편집 방침: 기자가 제시한 각도(세종→대전 투심 이동)를 **반박하지 않되, 근거 없는 인과도 쓰지 않는다.**
   확인된 수치와 그로부터 읽히는 국면 차이까지만 서술하고, 자금 이동 여부는 언급하지 않는다
   (매수자 거주지 자료가 국토부 공개항목에 없어 확인 자체가 불가). 해석은 읽는 쪽에 맡긴다.
"""
...
TITLE = "대전 선도지구 발표 엿새, 세종과 온도차 벌어졌다 — 콕집 DB로 본 충청권 매물 추이"
SUBTITLE = "발표 전엔 나란히 늘던 매물, 대전만 -22.5%…세종은 -0.5% · 호가 상승률은 8배 차"

LEAD = (
    "7월 15일 대전 노후계획도시 정비 선도지구 발표를 계기로 대전과 세종의 주택시장 온도차가 "
    "뚜렷해진 것으로 나타났다. 부동산 데이터 플랫폼 콕집(koczip.com)이 대전 선도지구 6개 단지와 "
    "세종시 정부청사 생활권 7개 동 103개 단지의 매물을 일자별로 전수 비교한 결과, 발표 전날인 "
    "7월 14일 대비 7월 20일 매매 광고매물은 대전이 227건에서 176건으로 22.5% 급감한 반면 "
    "세종은 15,385건에서 15,302건으로 0.5% 줄어드는 데 그쳤다. 주목할 점은 발표 직전까지 두 지역이 "
    "같은 방향으로 움직이고 있었다는 것이다. 발표 전 한 달간 매매 광고매물은 대전이 4.6%, 세종이 "
    "4.1% 늘어 증가율이 사실상 같았다. 온도차는 발표를 기점으로 벌어졌다."
)
...
BODY = [
    ...
    ("세종은 동별로도 ±2% 안팎 — 평시 등락 범위",
     "세종 청사권을 동 단위로 쪼개 보면 편차는 크지 않다. 발표 전날 대비 7월 20일 "
    ...
    ("호가에서도 온도차 — 상승률 8배",
     ...
     "매매뿐이라는 점도 확인된다."),
    ("1년 시계로 보면 — 가격 상승률은 대전이 앞섰다",
     "시야를 1년으로 넓히면 두 도시의 아파트 평균 실거래가는 다른 폭으로 움직였다. 국토교통부 실거래 "
     "신고 기준 아파트 매매 평균가는 대전이 2025년 8월 3억 4,312만 원에서 2026년 6월 3억 6,977만 "
     "원으로 7.8% 오른 반면, 세종은 5억 631만 원에서 5억 698만 원으로 0.1% 움직이는 데 그쳤다. "
     "거래량은 양쪽 모두 상반기 정점 이후 줄었다. 월별 매매 신고 건수는 2026년 1월 세종 589건·대전 1,736건으로 "
     "정점을 찍은 뒤 6월 세종 356건·대전 1,262건으로 각각 39.6%, 27.3% 감소했다."),
    ("온도차를 만든 것은 재건축 기대",
     "이번 데이터가 보여주는 것은 두 지역이 서로 다른 국면에 들어섰다는 점이다. 대전 선도지구는 발표라는 "
     "분명한 계기에 엿새 만에 매물이 22.5% 줄고 호가가 1.60% 오르며 즉각 반응했다. 재건축 기대가 "
     "가격에 실리기 시작한 국면으로 읽힌다. 반면 세종 청사권은 같은 기간 매물 -0.5%, 호가 +0.19%로 "
     "움직임이 크지 않았다. 1년 시계에서도 아파트 평균 실거래가는 대전이 7.8% 오르는 동안 세종은 "
     "0.1% 변동에 그쳐, 두 도시의 가격 상승률 격차가 벌어지는 구간이라는 점이 확인된다."),
    ("해석 시 유의점",
...
QUOTE = ("조용호 콕집 공동창업자는 “정비사업 발표가 시장에 만드는 파장은 지금까지 체감과 소문으로 "
         "이야기돼 왔다”며 “일자별 매물과 호가 데이터는 어느 지역이 언제, 얼마나 반응했는지를 숫자로 "
         "보여준다. 소비자와 시장이 각자 판단할 근거가 되도록 데이터를 계속 공개하겠다”고 말했다.")
