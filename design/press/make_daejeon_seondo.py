...
수치는 2026-07-24 complex_daily_agg 실측(단지 6곳: 한가람·공작한양·목련·크로바·보람·삼익소월).
※ 현행화(2026-07-24): 관측창을 발표 후 아흐레(~7/24)로 연장. 발송본(발표후동향·박재영 기자)과 수치·프레임 정합.
...
# ── 원고 ──
TITLE = "발표 아흐레 만에 매물 40%가 사라졌다 — 대전 선도지구 6개 단지, 콕집 DB로 본 발표 전후"
SUBTITLE = "콕집, 일자별 매물 전수 분석 — 광고매물 227→135건, 잠긴 것은 매매뿐 · 통합 호가는 아직 보합"

LEAD = (
    "국토교통부와 대전광역시가 7월 15일 노후계획도시 정비 선도지구 3개 구역(둔산지구 13·14구역, 송촌지구 6구역) 선정을 발표한 직후, 대상 단지의 매매 매물이 빠르게 회수되고 있는 것으로 "
    "나타났다. 부동산 데이터 플랫폼 콕집(koczip.com)이 자체 구축한 매물·실거래 "
    "데이터베이스로 대상 6개 단지(한가람·공작한양·목련·크로바·보람·삼익소월, 총 7,797가구)를 일자별로 "
    "전수 추적한 결과다. 발표 전날인 7월 14일 227건이던 6개 단지 매매 광고매물은 7월 24일 135건으로, "
    "발표 후 아흐레 만에 92건(40.5%) 줄었다. 같은 집의 중복 광고를 하나로 합친 ‘실매물’ 기준으로도 "
    "144건에서 99건으로 31.3% 감소했다. 반면 남은 매물의 호가는 아직 뚜렷하게 움직이지 않았다."
)

BODY = [
    ("대상 단지는 어떤 곳인가 — 전 단지 준공 30년 초과, 둔산은 발표 전후 신고가 경신",
     ...
     "점이다. 둔산 단지들은 올해 나란히 역대 최고가를 갈아치웠다. 목련이 4월 17일 15억 5,000만 원, "
     "한가람이 4월 28일 6억 2,000만 원, 크로바가 5월 19일 23억 5,000만 원으로 상반기에 신고가를 새로 "
     "썼고, 공작한양은 발표 직후인 7월 20일 6억 7,000만 원으로 역대 최고가를 다시 경신했다. 선도지구 "
     "기대감이 이미 가격에 실리고 있었고, 발표가 그 흐름에 불을 붙인 셈이다."),
    ("발표 전엔 늘던 매물이 발표 직후 급감 — 열흘 연속 회수",
     "매물 감소가 발표 이전부터 이어진 흐름이 아니라는 점이 데이터로 확인된다. 발표 전 한 달간(6월 18일~"
     "7월 14일) 6개 단지 광고매물은 217건에서 227건으로 오히려 늘고 있었다. 증가 추세가 발표 당일(7월 "
     "15일, 220건)을 기점으로 꺾여 16일 206건, 19일 187건, 22일 158건을 거쳐 24일 135건까지 열흘 연속 "
     "감소했다. 재건축 기대감이 커지자 매도자들이 내놓았던 집을 거둬들이는 ‘매물 잠김’이 일자별 데이터에 "
     "그대로 찍힌 것이다. 회수 강도는 구역별로 달랐다. 둔산 13·14구역이 각각 50.6%, 47.6% 줄어 절반 "
     "안팎으로 잠겼고(13구역 77→38건, 14구역 84→44건), 단지별로는 목련이 23건에서 7건(-69.6%)으로 감소 "
     "폭이 가장 컸다. 송촌지구 6구역은 -19.7%로 상대적으로 완만했다. 올해 신고가를 경신한 둔산 쪽이 더 "
     "세게 잠겼고, 신고가가 2023년에 머물러 있는 보람(4억 5,000만 원, 2023년 11월)·삼익소월(2억 "
     "9,800만 원, 2023년 9월)이 속한 송촌지구는 상대적으로 덜 민감했다 — 기대감의 온도차가 매물 회수 "
     "강도에도 그대로 나타났다."),
    ("호가는 아직 뚜렷하게 움직이지 않았다",
     "매물이 잠기는 동안에도 6개 단지 평균 매매 호가는 7월 14일 7억 6,800만 원에서 7월 24일 7억 6,100만 "
     "원으로 큰 변화가 없었다. 단지별로는 목련이 13억 3,000만 원에서 15억 1,000만 원으로 올랐으나, 이는 "
     "저가 매물이 회수되고 남은 매물이 대형 위주로 바뀐 구성효과이며(매물 15→7건), 나머지 단지는 대체로 "
     "보합이었다. 개별 매물 중에는 종전 신고가를 웃도는 호가(크로바 189타입 최저 호가 24억 원→25억 5,000만 "
     "원 등)도 나오지만, 매물이 크게 줄어든 상태에서 산출된 값이라 표본이 얇다. 이 호가가 실제 거래가격으로 "
     "이어질지는 발표 후 계약분의 신고가 쌓이는 다음 달 이후에야 확인된다."),
    ("전세 매물은 그대로 — ‘팔 사람’만 멈췄다",
     "같은 기간 6개 단지의 전세 매물은 46건에서 52건으로 오히려 늘었다. 잠긴 것은 매매뿐이다. 거주 수요가 "
     "빠져나간 것이 아니라, 소유자들이 값이 더 오를 것으로 보고 매도만 보류하고 있다는 뜻으로 읽히는 "
     "대목이다."),
    ("해석 시 유의점",
     "이번 수치는 발표 후 아흐레간(7월 24일 기준)의 반응이다. 매물 감소에는 호가를 다시 부르기 위한 회수와 "
     "실제 계약 성사가 섞여 있을 수 있다. 발표 이후 계약분의 실거래 신고는 아직 대부분 접수되지 않았고(신고 "
     "기한 30일, 6개 단지 7월 신고는 10건), 매물 잠김이 일시적 관망인지 지속될 흐름인지는 더 지켜봐야 한다. "
     "다만 광고 건수가 아니라 중복 광고를 합친 실매물 기준으로도 144건에서 99건으로 같은 방향의 감소가 "
     "확인돼(-31.3%), 착시가 아닌 실제 매물 잠김이라는 점은 분명하다."),
...
METHOD = (
    ...
    "실거래. 기간: 2026-06-17~07-24 일자별. 광고매물=포털 노출 광고 건수, 실매물=동일 주택의 중복 광고를 합친 수, "
    "평균 호가=매물수 가중평균. 신고가·실거래 평균은 국토교통부 실거래 신고 자료(해제거래 제외) 기준. 호가는 매도 "
    "희망가로 실거래 가격이 아님. 준공연도·세대수 포함 전 수치는 콕집 자체 구축 DB 실측(2026-07-24)."
)

# ── 실측 데이터(2026-07-24 DB) ──
DAILY = [  # (날짜, 광고매물, 실매물) — 6개 단지 합계. 6/28 실매물은 스냅샷 결측.
    ...
    ("07-15", 220, 137), ("07-16", 206, 134), ("07-17", 195, 128), ("07-18", 191, 126),
    ("07-19", 187, 126), ("07-20", 176, 117), ("07-21", 169, 114), ("07-22", 158, 110),
    ("07-23", 142, 101), ("07-24", 135, 99),
]

# 단지별 종료값(7/24)은 발송본(발표후동향, 박재영 기자)의 원자료 표와 일치시킴.
# 7/24는 수집 진행일이라 하루 중 변동 — 발송본 시점(오전) 스냅샷 기준으로 고정.
TABLE = [  # 구역, 단지(위치·준공), 광고 7/14→7/24, 증감, 실매물 7/14→7/24 — 전부 콕집 DB 실측
    ("둔산 13", "목련 (둔산동 · 1993)", "23 → 7", "-69.6%", "15 → 7"),
    ("둔산 13", "크로바 (둔산동 · 1992)", "54 → 31", "-42.6%", "28 → 21"),
    ("둔산 14", "한가람 (탄방동 · 1991)", "31 → 13", "-58.1%", "25 → 11"),
    ("둔산 14", "공작한양 (탄방동 · 1992)", "53 → 31", "-41.5%", "31 → 22"),
    ("송촌 6", "보람 (법동 · 1995)", "30 → 27", "-10.0%", "20 → 19"),
    ("송촌 6", "삼익소월 (법동 · 1993)", "36 → 26", "-27.8%", "25 → 19"),
    ("합계", "6개 단지 (7,797가구)", "227 → 135", "-40.5%", "144 → 99"),
]
...
def make_chart():
    ...
    ticks = [i for i, d in enumerate(DAILY) if d[0] in ("06-17", "06-24", "07-01", "07-08", "07-15", "07-20", "07-24")]
    ax.set_xticks(ticks)
    ...
    ax.set_title("대전 선도지구 6개 단지 매매 매물 추이 (2026.6.17~7.24, 콕집 DB)", fontsize=10.5, fontweight="bold", pad=8)
    fig.tight_layout()
...
def make_docx(out: Path):
    ...
    h = d.add_paragraph(); r = h.add_run("■ 단지별 개요·매매 매물 변화 (발표 전일 7/14 → 7/24, 콕집 DB)")
...
def make_pdf(out: Path):
    ...
    story = [
        ...
        chart,
        Spacer(1, 4 * mm),
        Paragraph("■ 단지별 개요·매매 매물 변화 (발표 전일 7/14 → 7/24, 콕집 DB)", ss["head"]),
        tbl,
        Spacer(1, 2 * mm),
    ]
    for head, body in BODY:
