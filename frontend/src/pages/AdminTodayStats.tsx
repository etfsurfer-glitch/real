...
export default function AdminTodayStats() {
  ...
  const load = useCallback(() => {
    if (!token || !API) return;
    setLoading(true);
    fetch(`${API}/admin/today-stats`, { headers: { Authorization: `Bearer ${token}` } })
      .then((r) => { if (!r.ok) throw new Error(r.status === 401 || r.status === 403 ? "관리자 권한이 필요해요" : `오류 ${r.status}`); return r.json(); })
  ...
  return (
    <div className="ats">
      ...
      <div className="ats-section">인기 페이지</div>
      <div className="ats-paths">
        {d.top_paths.map((p, i) => (
          <div key={i} className="ats-path"><span className="ats-path-r">{i + 1}</span><span className="ats-path-p">{p.path}</span><span className="ats-path-n">{p.n.toLocaleString()}</span></div>
        ))}
      </div>
      <p className="muted" style={{ fontSize: 11.5, marginTop: 12 }}>방문자=오늘 활동한 고유 IP · 최대 동시작업자=10분 구간 최대 동시 IP · 신규가입=오늘 약관동의 기준.</p>
    </div>
  );
}
