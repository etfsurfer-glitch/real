...
# 건축물대장 주용도가 이 중 하나면 '주거/숙박 타워'(상가는 그 저층부) → 상가·사무실 총층·주차
# 대조 부적합(주상복합·단지내상가). 근린생활·판매·업무 등 상업용 대장이면 대조 유효(살림).
_RESIDENTIAL_LEDGER_PURPS = ("공동주택", "아파트", "오피스텔", "숙박", "연립", "다세대",
                             "단독", "기숙사", "도시형생활")
...
def audit_listing(f: dict, *, cp_autofilled: bool = False) -> dict:
    ...
    if rtype in _RESIDENTIAL_TYPES:
        led_comparable = True
    else:
